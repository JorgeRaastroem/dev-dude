...
servers available for this session. If available, prefer indexer tools to validate that test
code matches production code.
...
- Verify that test code matches production code using the code indexer tools (if available) or by reading the source files
...
- ALWAYS validate that test code matches production code before presenting tests to the user
- Use code indexer tools (if available) or read source files directly with the view tool
...
- Investigate and identify how tests are executed in the current repository:
  ...
  - Check for test runner configurations (Jest, Vitest, pytest, unittest, etc.)
