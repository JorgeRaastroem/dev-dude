---
name: code-flow-analyzer-copilot
...
servers available for this session. If available, prefer indexer tools for symbol lookup, code
search, and dependency tracing over raw grep/glob when the indexer provides the capability.
