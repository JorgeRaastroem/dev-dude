---
name: feature-implementer-copilot
...
servers available for this session. If available, prefer indexer tools for code investigation
tasks over raw grep/glob when the indexer provides the capability.
...
- Use the code indexer tools (if available) or grep/glob/view to investigate the current codebase structure and patterns
...
- Format this document for optimal consumption by a sub-agent with clear, structured instructions
...
**Code Indexer Tools**: Use the code indexer MCP tools provided in your task prompt (if available) for code investigation tasks including:
