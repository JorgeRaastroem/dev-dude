...
For Copilot, invoke `code-flow-analyzer-copilot` and `feature-validator-copilot` and pass each
agent's reasoning-agnostic frontmatter `model` alias to its task call.
