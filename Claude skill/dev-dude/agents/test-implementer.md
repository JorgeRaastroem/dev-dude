...
> **Scope boundary**: This agent handles ALL test-related work. This includes writing new tests, improving coverage, AND updating existing tests that break due to production code changes (e.g., changed method signatures, renamed classes, updated logic). If the only files being changed are test files, this is the correct agent regardless of what triggered the change.
